...
public class SinavController extends BaseController {
	...
	@GetMapping(value = "/")
	public ResponseEntity<GenericServerResponse> getAllSeinavByKullaniciId(@PathVariable Long kullaniciId) {
		List<SinavDTO> sinavDTOList = sinavService.findAllByKullaniciId(kullaniciId);
	...
	public ResponseEntity<GenericServerResponse> deleteSertifika(@PathVariable Long kullaniciId, @PathVariable Long sinavId) {
		...
		return createResponseForSuccess(HttpStatus.CREATED, responseSinavDTO, responseSinavDTO.getProfil().getKullaniciId() + " nolu sınav başarıyla silindi");
	}
}